...
function jimpitanExportRows(startMonth,endMonth,rtFilter=state.reportRt){
 const rows=state.reportTransactions||[],selectedRt=rtFilter==='all'?'all':Number(rtFilter);
 return Array.from({length:endMonth-startMonth+1},(_,offset)=>{const month=startMonth+offset;const nominal=rows.filter(x=>(selectedRt==='all'||Number(x.kk?.rt)===selectedRt)&&x.jimpitan_schedule?.tanggal?.slice(5,7)===String(month).padStart(2,'0')).reduce((sum,x)=>sum+Number(x.nominal||0),0);return {month,monthName:MONTHS[month-1],nominal};});
}
function openJimpitanExport(){
 if(!state.reportTransactions?.length)loadReport();
 modal(`<div class="modalCard compact"><div class="modalHead"><div><div class="eyebrow">EXPORT JIMPITAN</div><h3>Rekap Nominal</h3><p class="muted">Tanpa nama KK. Pilih RT dan satu bulan atau rentang bulan.</p></div><button class="modalClose" data-close-modal>×</button></div><label>RT yang diexport<select id="exportRt"><option value="all">Semua RT</option>${state.rts.map(r=>`<option value="${r.id}" ${String(state.reportRt)===String(r.id)?'selected':''}>${esc(r.nama)}</option>`).join('')}</select></label><label class="exportAllLabel"><input id="exportAllMonths" type="checkbox"><span>Semua bulan tahun ${state.reportYear}</span></label><div class="twoCol"><label>Dari bulan<select id="exportStartMonth">${MONTHS.map((m,i)=>`<option value="${i+1}">${m}</option>`).join('')}</select></label><label>Sampai bulan<select id="exportEndMonth">${MONTHS.map((m,i)=>`<option value="${i+1}" ${i===11?'selected':''}>${m}</option>`).join('')}</select></label></div><div class="modalActions"><button class="secondary" data-close-modal>Batal</button><button class="secondary" id="exportJimpitanCsv">CSV</button><button class="primary" id="exportJimpitanPdf">PDF A4</button></div></div>`, 'exportRt');
 ...
 const exportWithRt=async(type)=>{const range=getRange();if(!range)return;const selectedRt=document.querySelector('#exportRt').value,previousRt=state.reportRt;state.reportRt=selectedRt;await loadReport();if(type==='csv')exportJimpitanCsv(...range,selectedRt);else exportJimpitanPdf(...range,selectedRt);state.reportRt=previousRt;await loadReport();};
...
function exportJimpitanCsv(startMonth,endMonth,rtFilter=state.reportRt){
 const rows=jimpitanExportRows(startMonth,endMonth,rtFilter),rt=rtFilter==='all'?'Semua RT':rtName(rtFilter),csv=[['Rekap Jimpitan',`${rt} - ${state.reportYear}`],['Bulan','Total Nominal'],...rows.map(row=>[row.monthName,row.nominal])].map(row=>row.map(value=>`"${String(value).replace(/"/g,'""')}"`).join(',')).join('\r\n');
...
function exportJimpitanPdf(startMonth,endMonth,rtFilter=state.reportRt){
 ...
 const doc=new jsPDF({orientation:'portrait',unit:'mm',format:'a4'}),pageWidth=210,left=18,right=192,rowHeight=10,total=rows.reduce((sum,row)=>sum+row.nominal,0),formatMoney=value=>`Rp ${new Intl.NumberFormat('id-ID').format(value)}`;
 doc.setFont('helvetica','bold');doc.setFontSize(17);doc.text('Rekap Jimpitan',left,22);doc.setFont('helvetica','normal');doc.setFontSize(10);doc.text(`${rt} | ${program} | Tahun ${state.reportYear}`,left,30);
 let y=42;doc.setFillColor(235,235,235);doc.rect(left,y,pageWidth-left-(210-right),rowHeight,'F');doc.setDrawColor(120,120,120);doc.rect(left,y,pageWidth-left-(210-right),rowHeight);doc.setFont('helvetica','bold');doc.text('Bulan',left+4,y+6.5);doc.text('Total Nominal',right-4,y+6.5,{align:'right'});doc.setFont('helvetica','normal');
 rows.forEach(row=>{y+=rowHeight;doc.rect(left,y,pageWidth-left-(210-right),rowHeight);doc.text(row.monthName,left+4,y+6.5);doc.text(formatMoney(row.nominal),right-4,y+6.5,{align:'right'});});
 y+=rowHeight;doc.setFont('helvetica','bold');doc.rect(left,y,pageWidth-left-(210-right),rowHeight);doc.text('TOTAL',left+4,y+6.5);doc.text(formatMoney(total),right-4,y+6.5,{align:'right'});doc.save(`rekap-jimpitan-${state.reportYear}-${startMonth}-${endMonth}.pdf`);closeModal();
}
