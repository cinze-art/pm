...
function exportJimpitanPdf(startMonth,startYear,endMonth,endYear,rtFilter=state.reportRt){
 ...
 const singleMonth=months.length===1,groupRows=[...groups.values()],doc=new jsPDF({orientation:singleMonth?'portrait':'landscape',unit:'mm',format:'a4'}),formatMoney=value=>`Rp ${new Intl.NumberFormat('id-ID').format(value)}`,page=singleMonth?{left:10,right:200,top:30,bottom:280}:{left:7,right:290,top:27,bottom:197},fixedWidth=singleMonth?8+22+82+25:8+18+48+20,monthWidth=(page.right-page.left-fixedWidth)/months.length,headerHeight=singleMonth?12:8,rowHeight=Math.max(singleMonth?3:1.4,Math.min(singleMonth?8:6,(page.bottom-page.top-headerHeight-8)/(groupRows.length+1))),fontSize=singleMonth?Math.min(7,rowHeight<4?4:6):(rowHeight<2.5?3:5),total=groupRows.reduce((sum,row)=>sum+row.total,0);
 ...
 if(singleMonth){
  const split=Math.ceil(groupRows.length/2),leftRows=groupRows.slice(0,split),rightRows=groupRows.slice(split),tableGap=6,tableWidth=(page.right-page.left-tableGap)/2,columns=[{label:'No',width:8},{label:'Nama KK',width:tableWidth-8-24-20},{label:MONTHS[startMonth-1],width:24},{label:'TOTAL',width:20}],tableHeaderHeight=10,tableRowHeight=Math.min(8,(page.bottom-page.top-tableHeaderHeight-8)/Math.max(leftRows.length,rightRows.length));
  const renderTable=(tableRows,startIndex,x)=>{let y=page.top,cellX=x;doc.setFont('helvetica','bold');doc.setFontSize(5.5);columns.forEach(column=>{doc.setFillColor(255,255,255);doc.setDrawColor(70,70,70);doc.rect(cellX,y,column.width,tableHeaderHeight,'FD');doc.text(column.label,cellX+column.width/2,y+6,{align:'center',maxWidth:column.width-1});cellX+=column.width;});tableRows.forEach((row,index)=>{y=page.top+tableHeaderHeight+tableRowHeight*index;cellX=x;const cells=[String(startIndex+index+1),row.kkName,row.values[`${startYear}-${startMonth}`]?formatMoney(row.values[`${startYear}-${startMonth}`]):'—',formatMoney(row.total)];columns.forEach((column,columnIndex)=>{doc.setFont('helvetica',columnIndex===3?'bold':'normal');doc.rect(cellX,y,column.width,tableRowHeight);doc.text(cells[columnIndex],cellX+(columnIndex===1?1:column.width/2),y+tableRowHeight*.68,{align:columnIndex===1?'left':'center',maxWidth:column.width-2});cellX+=column.width;});});};
  const rightX=page.left+tableWidth+tableGap;renderTable(leftRows,0,page.left);renderTable(rightRows,split,rightX);doc.setFont('helvetica','bold');doc.setFontSize(7);doc.text(`TOTAL PERIODE: ${formatMoney(total)} | ${groupRows.length} KK`,page.left,Math.min(page.bottom+5,page.top+tableHeaderHeight+tableRowHeight*Math.max(leftRows.length,rightRows.length)+5));doc.save(`rekap-jimpitan-${startYear}-${startMonth}-${endYear}-${endMonth}.pdf`);closeModal();return;
 }
 const columns=[{label:'No',width:singleMonth?12:8},{label:'RT',width:singleMonth?22:18},{label:'Nama KK',width:singleMonth?82:48},...months.map(month=>({label:MONTHS[month.month-1],year:String(month.year),width:monthWidth})),{label:'TOTAL',width:singleMonth?25:20}];let x=page.left;doc.setTextColor(0,0,0);doc.setFont('helvetica','bold');doc.setFontSize(Math.min(fontSize,4.5));columns.forEach(column=>{doc.setFillColor(255,255,255);doc.setDrawColor(70,70,70);doc.rect(x,page.top,column.width,headerHeight,'FD');const headerText=column.year?`${column.label} ${column.year}`:column.label;doc.text(headerText,x+column.width/2,page.top+headerHeight*.68,{align:'center',maxWidth:column.width-1});x+=column.width;});doc.setTextColor(0,0,0);
